...
m = match(rownames(agg.rank),rownames(genesets.up[rowSums(genesets.up) >=3,]))
f.a = !is.na(m)
local = agg.rank[f.a,f.a]
...
ggdendrogram(consTree,rotate = TRUE,labels = F, theme_dendro = FALSE) +
  geom_hline(aes(yintercept = max(dist(temp))*0.38)) +
  theme_classic()
...
#Plot 4 - global v cluster node degrees (OPTIONAL)
...
pdf("Thesis figures/Chp2-Figure 2.pdf", height = 7,width = 8, useDingbats =  F)
plot_grid(ht,gtps, nrow = 2, rel_heights = c(2,1), labels = LETTERS[1], label_fontfamily = 'serif', label_size = 18)
dev.off()

save(df, gmat, res1, local, clusters,file = 'Coexpression-influenza-human-network and datasets.rdata')
